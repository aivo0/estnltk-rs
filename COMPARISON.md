...
| CSV loading (`ruleset.load()`) | Reads rules from CSV with typed columns (int, float, regex, string, callable, expression) | `rs_load_rules_csv` function with typed columns (int, float, regex, string, bool) | **Partial** |
| `CONVERSION_MAP` type coercions | `int`, `float`, `regex`, `string`, `callable`, `expression` | `int`, `float`, `regex`, `string`, `bool` | **Partial** |
| `rule_map` property | Maps patterns to grouped rules | `rule_map` property on `RsRegexTagger` and `RsSubstringTagger`: returns dict mapping pattern strings to lists of rule dicts | **Full** |
...
- Test merging (`merge_positive_tests`, `merge_negative_tests`, `merge_extraction_tests`) — Python-specific test infrastructure
...
**Features ported (ChoiceGroup):**
- Simple alternation: `rs_choice_group_pattern(patterns)` validates each pattern with resharp and produces `(?:pattern1|pattern2|...)`
- Compatible StringList merge: `rs_merged_string_lists_pattern(string_lists, ...)` merges multiple string lists with the same replacements into a single pattern with longest-first sorting — matching EstNLTK's `ChoiceGroup` optimization for compatible `StringList` children
- Per-list case sensitivity flags supported via `ignore_case_flags_per_list` parameter
...
| Invalid regex in CSV `regex` column | `regex.error` during `CONVERSION_MAP` processing | Error at `rs_load_rules_csv` time with line number, column name, and resharp parse error |
