...
use estnltk_regex_rs::types::{ConflictStrategy, MatchSpan, TaggerConfig};
...
/// Fresh rules are created each time (ExtractionRule is not Clone).
...
fn bench_substring_text_size(c: &mut Criterion) {
    ...
    for &size in &[1_000, 10_000, 100_000] {
        ...
        let rules: Vec<_> = SUBSTRING_PATTERNS[..10]
            .iter()
            .map(|p| make_substring_rule(p, HashMap::new(), 0, 0))
            .collect();
        let tagger =
...
fn generate_match_entries(n: usize) -> Vec<(MatchSpan, usize)> {
    // Generate n overlapping match entries for conflict resolution benchmarks.
    // Pattern: spans of length 5, starting every 3 positions → overlapping by 2.
    (0..n)
